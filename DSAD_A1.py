...
# This function can append the node to DLL     
...
# This will delete any node from DLL - [For future use]
...
def process_stock_prices(input_file, output_file, capacity=None):
    try:
        ...
        try:
            if not lines[0].strip().split('=')[0].strip() == 'stock_prices':
                raise ValueError(f"Stock Prices data is not available.")
            
            if not lines[1].strip().split('=')[0].strip() == 'moving_average_window':
                raise ValueError(f"Moving Average Window value is not available.")
            
            if not lines[2].strip().split('=')[0].strip() == 'threshold':
                raise ValueError(f"Threshold value is not available.")
        
            stock_prices = lines[0].strip().split('=')[1].strip().strip("[]")
            moving_average_window = lines[1].strip().split('=')[1].strip()
            threshold = lines[2].strip().split('=')[1].strip()
...
capacity = 1000  # capacity of the nodes can be changed here. For now, it's set to 1k, if entries exceed 1000 it will generate an error
